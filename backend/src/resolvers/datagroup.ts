...
export class DataGroupResolver {
    ...
    // Mutation to add a file to a data group
    @Mutation(() => DataGroupResponse)
    async addFileToDataGroup(
        @Arg("id") id: number,
        @Arg("file") file: DataFile,
    ): Promise<DataGroupResponse> {
        const dataGroup = await this.dataGroupRepo.findOne({ where: { id } });
        if (!dataGroup) {
            return {
        ...
        dataGroup.files.push(file);
        dataGroup.totalData = dataGroup.totalData + file.size;
        await this.dataGroupRepo.save(dataGroup);
        return { dataGroup };
    }
